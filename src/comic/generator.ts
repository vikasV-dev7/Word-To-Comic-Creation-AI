...
export async function generateComic(project: ComicProject): Promise<ComicScript> {
  const panels = await buildPanels(project)
...
async function fetchPanelImage(panel: Omit<PanelScript, 'imageUrl'>): Promise<string> {
  ...
  let scene_description = `Place: ${location}. Weather: ${weather}. Time: ${time}. Action: ${situation.description}. Scene mood: ${emotion}.`
  if (panel.dialogue) {
    scene_description += ` Dialogue: ${panel.speaker?.name || 'speaker'} says "${panel.dialogue.text}"`
  }
  ...
  const payload = {
    ...
    character_refs: panel.characters.map(c => ({
      name: c.name,
      description: `${c.age ? c.age + ' year old ' : ''}${c.gender || 'person'}, ${c.height ? c.height + ' tall' : ''}, wearing ${c.dressColor || ''} ${c.dress}. ${c.description || ''}`.trim()
    })),
    style_prompt: "High-quality anime illustration, beautiful detailed digital manga art, cinematic lighting, masterpiece, webtoon style, clean lineart",
    negative_prompt: "mutated, deformed, bad anatomy, text, watermark"
  }

  try {
    const res = await fetch('http://localhost:8000/api/generate/panel', {
      method: 'POST',
    ...
    console.error("Failed to generate panel image via backend", err)
  }
  ...
  return `https://image.pollinations.ai/prompt/${encodeURIComponent(scene_description)}?width=800&height=1000&seed=${seed}&nologo=true&private=true`
}

async function buildPanels(project: ComicProject): Promise<PanelScript[]> {
  const panels: PanelScript[] = []

  for (let sitIdx = 0; sitIdx < project.situations.length; sitIdx++) {
    const sit = project.situations[sitIdx]
    ...
    introPanel.imageUrl = await fetchPanelImage(introPanel)
    panels.push(introPanel)

    // One panel per dialogue
    for (let dialIdx = 0; dialIdx < sit.dialogues.length; dialIdx++) {
      const dial = sit.dialogues[dialIdx]
      ...
      dialPanel.imageUrl = await fetchPanelImage(dialPanel)
...
function distributeToPages(panels: PanelScript[]): PageScript[] {
